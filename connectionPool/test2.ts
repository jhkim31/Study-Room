...
for (const c of s) {    
    connectionPool.add({
        url: `https://apis.data.go.kr/1360000/AsosHourlyInfoService/getWthrDataList?serviceKey=${process.env.SERVICE_KEY}&pageNo=1&numOfRows=744&dataType=JSON&dataCd=ASOS&dateCd=HR&startDt=20231104&startHh=00&endDt=20231204&endHh=12&stnIds=${c}`,
        resolve: (d => {
            console.log(d.data);
        }),
        reject: (e => {})
